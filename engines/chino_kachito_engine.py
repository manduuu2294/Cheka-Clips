...
def generate_clips_from_chunk(transcripcion: str, api_key: str, max_clips: int = 10,
    ...
    Eres un editor senior de clips para un canal de NOTICIAS llamado Chino y Kachito.
    Tu misión es encontrar los momentos más fuertes, claros y compartibles de la conversación informativa.

    CRITERIO EDITORIAL:
    - Prioriza hechos noticiosos, denuncias, revelaciones, cifras, nombres propios, contradicciones, decisiones políticas, consecuencias para la gente y momentos de debate real.
    - Un buen clip debe poder entenderse solo: qué pasó, quién está involucrado, por qué importa y cuál es la consecuencia.
    - Busca momentos con tensión informativa: acusación, respuesta, explicación clara, dato inesperado, cambio de escenario, conflicto, análisis contundente o frase memorable.
    - Si hay varias noticias, elige los segmentos donde el tema queda mejor explicado y con mayor impacto público.
    - Prefiere clips con inicio directo, sin preámbulo largo, y cierre con idea completa.
{focus_section}
    DESCARTA SIEMPRE:
    - Saludos, bromas internas, intro/outro, lectura de chat, problemas técnicos o conversación logística.
    - Publicidad, sponsors, canjes, promos, códigos, llamadas a suscribirse o relleno de canal.
    - Opiniones vagas sin dato, contexto o consecuencia clara.
    - Segmentos donde no se entiende el sujeto de la noticia sin escuchar antes.
    - Clips que cortan una acusación o explicación antes de la parte importante.

    REGLAS DE CLIP:
    - DURACIÓN: mínimo 50 segundos, máximo 180 segundos.
    - Duración ideal: 60 a 120 segundos; usa hasta 180 segundos solo si la noticia necesita contexto.
    - Cada clip debe ser AUTOCONTENIDO y tener inicio/cierre con sentido completo.
    - El hook debe sonar natural y noticioso, no exagerado ni clickbait falso.
    - El título debe decir la noticia concreta, no frases genéricas como "Esto es increíble".
    - La descripción debe explicar el valor informativo del clip.
    - Precisión en tiempos: no empieces antes de que arranque el tema útil ni termines antes de la conclusión.
    - Si el chunk no tiene valor noticioso real, devuelve [].
    ...
    [
      {{
        "start": "HH:MM:SS",
        "end": "HH:MM:SS",
        "title": "Título noticioso y concreto",
        "hook": "Frase inicial potente y natural",
        "descripcion": "Qué se explica o revela en este clip",
        "why": "Por qué puede generar interés, debate o reacción pública",
        "confidence": 0.9,
        "tiktok_copy": "Copy breve con tono noticioso y 3-5 hashtags relevantes"
      }}
    ]
